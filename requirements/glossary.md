...
- **Build.** Stage 2 of the DSK pipeline: agent reads snapshot plus briefs, produces the library — both renditions (web-rendered layouts, examples, and content items) and library pages (the browser around them). See [outputs.md](outputs.md).
...
- **Rendition.** A web-rendered version of a layout, example, or content item from the snapshot, produced by `dsk:build` and stored as one HTML file each at `library/renditions/{layouts,examples,content}/<id>.html`. Reused by `dsk:compose` as starting templates when generating slides; tunable per-entry via `dsk:refine` when a rendition drifts from source. The actual product DSK delivers; the library pages are the browser around them. See [outputs.md](outputs.md).
