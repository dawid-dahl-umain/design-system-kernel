...
`Rect` (carried by every `Placeholder.position`) is the structural contract between snapshot and rendition. The build and align skills consume `Rect` directly — they transcribe each placeholder's position into the output medium per-rendition rather than approximating from a family-level template. This is why every engine emits the same `Rect` shape with unit-free fractions: the rendering side doesn't need to know which engine produced the snapshot, only that geometry is exact and per-placeholder. See `dsk/skills/build/SKILL.md` ("Reading the source screenshots and the snapshot data together") and `dsk/skills/align/SKILL.md` for the behavior-level rule.
