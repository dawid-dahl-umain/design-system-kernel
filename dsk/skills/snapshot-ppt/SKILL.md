...
- **`role`** (recommended when confident): one of `brand-mark`, `decorative`, `photographic`, `icon`, `background`. Determine by inspecting which design-system parts reference it: a small PNG with transparency referenced from the slide master and positioned near a corner is almost certainly a brand mark; an image referenced as a layout's background fill is a background; a large photographic image only on one or two layouts is decorative or photographic specimen. **If you cannot classify confidently, omit `role`** rather than guessing — the build agent can re-classify by inspection.
