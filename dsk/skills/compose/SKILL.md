...
- **Layout renditions** at `library/renditions/layouts/<layout-id>.html` — the actual starting templates. Generated by `dsk:build`.
- The snapshot in the company zone (`snapshot/snapshot.json` plus assets) — drives layout selection, DoF reasoning, and content-catalog matching.
...
Read `snapshot/snapshot.json` from the company zone. Pick the best matching layout from its `layouts` array. The chosen layout's `id` resolves to a rendition file at `library/renditions/layouts/<layout-id>.html` — that file is the starting template.
...
- **If the chosen layout's rendition file is missing** (deleted, partial build, never generated): tell the user the library is incomplete and recommend `/dsk:build` to regenerate. Do not re-render the layout inline as a fallback — that would silently diverge from what the library shows.
...
- **Don't change the rendition's styling, structure, or layout** — those decisions were made at build time and are part of the company's configured visual system. Filling placeholders is your job; restyling is not.
