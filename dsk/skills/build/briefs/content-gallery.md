...
- **A "compare to source" affordance for every entry.** Show the primary source screenshot from `snapshot/assets/content-screenshots/<id>.png` alongside the rendition (side-by-side by default; toggle acceptable). For content items this is especially important: charts, tables, and diagrams have many small structural details (axis ticks, legend placement, header backgrounds, arrow styles) that are easy to miss without visual comparison.
