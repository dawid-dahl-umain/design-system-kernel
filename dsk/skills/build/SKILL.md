...
**Treat the snapshot's structural geometry as authoritative.** The snapshot encodes the source's exact placeholder positions for every layout; the rendition transcribes those positions into the output medium, it does not approximate them. Read each rendition's geometry from its own snapshot entry and let those values drive the output directly. Sibling layouts often differ from each other by small amounts — a title slightly taller, a body slightly narrower, a footer at a different vertical proportion — and those differences are the design system being captured by the snapshot. Squashing them into one shared approximation across siblings discards the precision the source took the trouble to encode, and the drift compounds across the library. The screenshot tells you what the slide *looks like* (visual character — palette, imagery, decorative motifs); the snapshot tells you what the slide *is shaped like* (structural skeleton — placeholder positions, sizes, proportions). Both feed every rendition. Creative latitude lives in the styling on top of the skeleton, not in the skeleton itself. A common slip: writing position values for a placeholder without having opened the snapshot to read its entry; when you notice that, stop and read.

**At self-check, verify proportions, not just region presence.** A rendition can have every required region (title, body, picture, footer) and still be wrong if any region sits at the wrong proportion of the slide — a body that ends past the slide's natural bottom margin looks like overflow, a footer at the wrong vertical position can shift the whole composition's feel, a picture placeholder that's slightly too wide can crowd the text column next to it. Hold each region's bounding box against the source screenshot and confirm proportions correspond. Where they don't, the snapshot's geometry is the correction.

**Where they conflict, the screenshot wins.** This applies to type, composition, and character classifications; placeholder geometry is the snapshot's domain alone, with no conflict to resolve there (see the geometry rule above). This is the failure mode that has cost the most fidelity in past runs: the snapshot's `type` label is a structural classifier that may describe an *embedded element* rather than the slide's *overall composition*. A slide classified as `bar-chart` because it contains an embedded stacked-column chart can in fact be a five-metric four-region dashboard with that chart as one tile inside it. Generating from metadata alone produces a textbook bar chart and the source character is lost. The screenshot is the truth; the metadata corroborates and refines, it does not override.
...
- **Do not invent decoration the source clearly does not have.** This is the inverse risk. If a layout is genuinely minimal (e.g. a `title-cream` layout is just a cream wash with a title and the company's wordmark), keep the rendition minimal. The skepticism above applies when the screenshot looks impoverished *relative to the source's actual character*, not as license to embellish past what the source shows.
  ...
  **Equally, do not generalize a decoration from a few sampled layouts to the whole template.** Spot-checking a decorative motif on a title slide and concluding "all light-themed layouts have it" is the same failure class with a different shape: the agent reads N screenshots, sees a pattern, then applies it across the rest without reading those screenshots. Decorative motifs are usually family-specific in real templates; body-content layouts especially are often deliberate clean canvases. The per-rendition screenshot read in step 2 isn't a one-time calibration that establishes a library-wide rule — every rendition's screenshot is its own ground truth. When in doubt about whether a decoration applies to a given rendition, look at *that* rendition's screenshot, not at the pattern you inferred from others. The audit-trail note for each rendition has to reflect what's actually in *its* screenshot, not what the previous rendition had.
...
2. **Generate renditions.** One file per layout, one file per example, one file per content item, in the structure above. **Per rendition:** load its source screenshot *and* read its snapshot entry (placeholders, type, notes, ids), describe what's depicted using both — for matrix-structured sources, narrate the structure in prose before writing HTML — then write the file, **transcribing each placeholder's position directly from the snapshot entry rather than eyeballing values or reusing a sibling layout's geometry**. After writing, structurally self-check against the screenshot — **verify both regions (are the right ones present?) and proportions (do they sit at the same fractions of the slide as in the source?)**; see the "Reading the source screenshots and the snapshot data together" section above for the full rule. Then **emit a one-line self-check note in your chat output** confirming the rendition matches its source. The note's shape is up to you; the load-bearing constraint is specificity — the note has to name what the source actually shows in enough detail that you couldn't have produced it without looking at the screenshot. Vacuous notes ("rendition matches ✓") defeat the purpose. The act of writing a specific note is what forces the look. Only then move to the next rendition.
...
5. **Verify pass — invoke `dsk:align` as the final acceptance gate against the source.** Step 2's per-rendition audit trail is the in-flight defense; this is the library-level pass that catches anything that slipped through and any cross-rendition consistency issues that only become visible at library scale. The pass is implemented as a separate skill so it gets a fresh skill-boundary attention reset rather than being blurred into the tail of the generation steps; the discipline (per-rendition screenshot reads, audit-trail notes, no-batch rule, snapshot geometry as authoritative for placeholder positions, proportion-check at self-check, screenshot-wins-on-conflict-of-type-or-composition, no-decoration-overgeneralization) lives in that skill. Build is not done until align returns clean — if align fixes renditions in place or flags snapshot misclassifications, surface them in the run summary alongside the build's own outputs. Schema validation in step 4 cannot catch "rendition is off from source" on either axis (structure and character); only the align pass can.
