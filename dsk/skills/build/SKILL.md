...
5. **Verify pass — final acceptance gate against the source.** Step 2's per-rendition audit trail is the in-flight defense; this is the library-level pass that catches anything that slipped through and any cross-rendition consistency issues that only become visible at library scale. For every rendition tile across `layouts.html`, `examples.html`, and `content-gallery.html`, hold the rendition next to its source screenshot and confirm correspondence on both axes — **structure** (visible regions, dominant primitives, spatial relationships: rows, columns, grids, sidebars, headers) and **character** (palette and color, key imagery, brand marks like logos, wordmarks, and decorative motifs, overall visual feel). A rendition can fail either axis: same regions but wrong palette; same composition but missing the source's signature image; same layout but different brand mark — all fail. Fix and re-verify. Do not declare the build done until every tile would survive a user clicking compare-to-source without spotting an obvious gap. Schema validation in step 4 cannot catch "rendition is off from source" on either axis — only this self-check can. If you spot a misclassification at the snapshot level (e.g. a slide labelled `bar-chart` that is actually a comparison matrix), flag it in the run summary so the user can decide whether to re-snapshot with a corrected type. This is the acceptance gate for whether the library actually represents the company faithfully; treat it accordingly.
