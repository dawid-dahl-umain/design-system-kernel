...
**Pre-write narration for matrix-structured sources.** When the source has a row × column structure — matrices, comparison tables, multi-section grids, anything with a left-side label column and entity columns — do the structural narration *before* writing HTML, not after. List rows in order, columns in order, what fills each cell at the intersections, and any cells that span multiple rows or columns. Reconcile against the screenshot. Only then write code. The most expensive failure here is **transposition**: the source presents rows-as-sections (financials, regions, offerings, strengths) and columns-as-entities (companies, products, time periods); the rendition then presents the inverse — M vertical stacks each containing every section label, with the label column duplicated M times instead of appearing once on the left, and the cross-entity comparison the source affords is destroyed. Pre-write narration makes transposition obvious before any code is written; post-write self-check often catches it too late, when reshuffling every cell is the cost of fixing it. Whenever the source has a left-side label column, preserve the row-as-section, column-as-entity orientation.
...
2. **Generate renditions.** One file per layout, one file per example, one file per content item, in the structure above. **Per rendition:** load its source screenshot *and* read its snapshot entry (placeholders, type, notes, ids), describe what's depicted using both — for matrix-structured sources, narrate rows / columns / cells in prose before writing HTML — then write the file. After writing, structurally self-check against the screenshot (see the "Reading the source screenshots and the snapshot data together" section above), then **emit a one-line self-check note in your chat output** confirming the rendition matches its source (format like *"`comparison-matrix` — source: 5 rows (sections) × 4 entity columns + 1 left label column; rendition matches ✓"*). The note is the audit trail; the act of writing it forces you to actually look. Skipping or rubber-stamping is a known failure mode under bulk-generation pressure — don't. Only then move to the next rendition.
