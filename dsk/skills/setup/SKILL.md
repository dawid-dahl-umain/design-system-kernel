...
9. **Run the build stage** by invoking `dsk:build`.
   ...
   This produces `library/renditions/{layouts,examples,content}/<id>.html` (the actual slide renditions) plus `library/welcome.html`, `library/layouts.html`, `library/examples.html`, `library/content-gallery.html` (the browser pages). Build itself ends with an internal align pass (its own step 5 invokes `dsk:align`); setup adds an additional align pass below as a fresh thorough sweep at the post-setup moment.

10. **Run a closing alignment pass** by invoking `dsk:align`. Build has just finished and includes its own align pass internally, but setup runs align *again* as a separate skill invocation on the freshly-generated library. The justification is mechanical, not duplicative: a skill-boundary invocation gives the agent a fresh attention reset on every rendition that's harder to skim than the next-step-in-a-list framing, and the in-flight discipline during generation has, in practice, sometimes left drift that only a dedicated post-build pass catches. Align is idempotent — a clean library produces no edits — so the cost when build's internal pass already covered everything is just the read-through, which is the point.

   Before invoking, narrate. Suggested phrasing: "Build is done. Doing one more careful pass over every rendition now to make sure each one matches your source — comparing each slide to its screenshot and fixing anything that drifted." Then run align.

11. **Confirm completion to the user.** Point them at `library/welcome.html` as the entry point, using whatever consumption channel your runtime supports — host-native preview if the host AI Design Tool renders HTML inline, otherwise instruct them to open the file in a browser. Then suggest they start building decks via chat. If align surfaced any items that couldn't be fully aligned or any snapshot misclassifications, mention them here in plain English so the user knows what to follow up on.
...
- If align fails or surfaces gaps it can't close after build succeeds: build's output is preserved (renditions and library pages are on disk and usable); surface what align found and recommend `/dsk:align` for a re-run or `/dsk:refine` for any specific rendition the user wants to fix manually. Setup is still considered complete in this case.
